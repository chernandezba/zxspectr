...
{                                                 
  if (!(n%1024)) printf ("Convertido en %d%%\r",n*100/49152); 
}
...
void fleer(char *p,uint bytes,FILE *f)
{
  for (;bytes;bytes--)
	 *p++=fdarc(f);
}

void fescr(char *p,uint bytes,FILE *f)
{
  for (;bytes;bytes--)
	 fponc(*p++,f);
}
...
void main(void)
{
  ...
  strupr(origen);
  if (strstr(origen,".Z80")!=NULL) dest=1;
  ...
  setcache(output,'w');
  if (dest) {
	 fleer (&z80,sizeof(struct Z80),input);
	 if (!z80.PC_old) fleer(&z802,sizeof(struct Z802),input);
  }
  else fleer(&sp,sizeof(struct SP),input);
  if (dest) {
	 if (!z80.PC_old && z802.hardware) {
		...
		sp.DE_=z80.DE_;
		sp.HL_=z80.HL_;
		sp.AF=z80.F+((uint)z80.A << 8);
		sp.AF_=z80.F_+((uint)z80.A_ << 8);
		sp.IX=z80.IX;
		sp.IY=z80.IY;
		...
		fescr(&sp,sizeof(struct SP),output);
		bytes_escritos=0;
		for (m=0;m<3;m++) {
		  segmento=m;
		  if (!z80.PC_old) { 
	 ...
	 fescr(&z80,sizeof(struct Z80),output);
	 for (n=0;n<49152;n++) {
		pon_porcentaje(n);
		byte=fdarc(input);
		fponc(byte,output);
